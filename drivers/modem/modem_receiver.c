/** @file
 * @brief Modem UART network receiver driver
 *
 * A Modem UART network receiver driver allowing application to handle all
...
static void mdm_receiver_flush(struct mdm_receiver_context *ctx)
{
	...
	/* Drain the fifo */
	while (uart_fifo_read(ctx->uart_dev, &c, 1)) {
		continue;
	}
...
static void mdm_receiver_isr(struct device *uart_dev)
{
	...
	while (uart_irq_update(ctx->uart_dev) &&
	       uart_irq_is_pending(ctx->uart_dev)) {
		if (!uart_irq_rx_ready(ctx->uart_dev)) {
			continue;
		}
		      ...
		      u8_t *buf, size_t size, size_t *bytes_read)
{
	...
	ret = k_pipe_get(&ctx->uart_pipe, buf, size, bytes_read, 1, K_NO_WAIT);
	return ret;
}
