...
static struct wncm14a2a_socket *socket_from_netctx(struct net_context *context)
{
	int i;
...
static void socket_reset(struct wncm14a2a_socket *sock)
{
	if (!sock) {
		return;
	}
			...
			socklen_t addrlen)
{
	...
	sock = socket_from_netctx(context);
	if (!sock) {
