...
class Nios2BinaryRunner(ZephyrBinaryRunner):
    ...
    def do_add_parser(cls, parser):
        ...
        parser.add_argument('--nios2-cpu-sof',
                            help='location of the CPU .sof data')
    ...
    def create_from_args(command, args):
        cwd = os.getcwd()
        hex_name = path.join(cwd, 'zephyr', args.kernel_hex)
        elf_name = path.join(cwd, 'zephyr', args.kernel_elf)

        return Nios2BinaryRunner(hex_name=hex_name, elf_name=elf_name,
                                 cpu_sof=args.nios2_cpu_sof,
