...
class RunnerCaps:
    ...
    - flash_addr: whether the runner supports flashing to an
      arbitrary address. Default is False.
    '''
...
class ZephyrBinaryRunner(abc.ABC):
    ...
    **Note**: these APIs are still evolving, and will change!
    ...
    def add_parser(cls, parser):
        ...
        * --dt-flash
        ...
        parser.add_argument('--kernel-elf', required=True,
                            help='Kernel binary in .elf format')
        parser.add_argument('--kernel-hex', required=True,
                            help='Kernel binary in .hex format')
        parser.add_argument('--kernel-bin', required=True,
                            help='Kernel binary in .bin format')

        # Optional options.
        parser.add_argument('--dt-flash', default=False,
                            action='store_true',
                            help='''If given, use configuration generated by
                            device tree (DT) to compute flash addresses. Note:
                            this is not supported by all runners.''')
    ...
    def get_flash_address(cls, args, build_conf, default=0x0):
        ...
        However, if the class's capabilities() implementation does not
        support flash_addr, a ValueError is raised instead.

        Otherwise (when args.dt_flash is false), the default value is
        returned.'''
        if args.dt_flash:
            caps = cls.capabilities()
            if not caps.flash_addr:
                msg = ('Runner "{}" does not support flashing to an arbitrary '
                       'address, but devicetree-based flashing (--dt-flash) '
                       'was requested. The runner must be extended to support '
                       'this option.')
